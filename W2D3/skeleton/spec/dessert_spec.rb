...
describe Dessert do
  let(:chef) { double("chef", name: "Anthony") }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect{Dessert.new("icecream", "chocho", chef)}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#eat" do
    it "subtracts an amount from the quantity" do
      dessert.eat(4)
      expect(dessert.quantity).to eq(36)
    end

    it "raises an error if the amount is greater than the quantity" do
      expect { dessert.eat(550) }.to raise_error("not enough left!")
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Anthony the Great Baker")
      expect(dessert.serve).to eq("Chef Anthony the Great Baker has made 40 brownies!")
    end
  end
